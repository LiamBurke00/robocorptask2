...
def download_orders_file():
    """Downloads orders file from the give URL"""
    http = HTTP()
    http.download("https://robotsparebinindustries.com/orders.csv", overwrite=True)

def order_another_bot():
    """Clicks order another button to order another bot"""
    page = browser.page()
    page.click("#order-another")

def clicks_ok():
    """Clicks ok whenever a new order is made for bots"""
    page = browser.page()
    page.click('text=OK')

def fill_and_submit_robot_data(order):
    """Fills in robot order details and clicks the 'Order' button"""
    page = browser.page()
    head_names = {
        "1" : "Roll-a-thor head",
...
def store_receipt_as_pdf(order_number):
    """stores robot order receipt as pdf"""
    page = browser.page()
...
def archive_receipts():
    """Archives the receipt pdfs into a single zip archive"""
    lib = Archive()
